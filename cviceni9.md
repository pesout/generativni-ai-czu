...
- **Bot je z principu veřejný a defaultně nechráněný.** Username (`@<jmeno>_personal_bot`) je dohledatelný v Telegramu -- bez ochrany by mu mohl napsat kdokoliv, kdo username uhodne nebo najde, a každá taková zpráva projde rovnou do LLM agenta včetně všech jeho skillů (mail, soubory, shell). Tohle je **primární obrana, důležitější než tajnost tokenu** -- token totiž nikomu vědomě neposíláte, ale username svého bota časem někomu ukážete.

  Ochrana má dva kroky -- najít své Telegram user ID a doplnit ho do allowlistu:

  1. **Zjistěte si své numerické Telegram user ID.** Je to číslo jako `123456789`, *ne* `@username`. Nejjednodušší cesta: v Telegramu napište zprávu botovi [@userinfobot](https://t.me/userinfobot) (nebo `@getmyid_bot`) -- odpoví vám hláškou *"Id: 123456789"*. Pokud nechcete použít třetí stranu, napište cokoli vlastnímu botovi a user ID najdete v `docker compose logs --tail=20` v `~/openclaw` v poli `from.id` přicházejícího Update.
  2. **Doplňte ID do allowlistu přes Codex.** V `~/openclaw` mu řekněte: *"doplň user ID `123456789` do allowlistu Telegram channelu v `openclaw.json` (přesný klíč si najdi v aktuální dokumentaci -- typicky `channels.telegram.allowFrom` nebo `dmPolicy: "pairing"`) a restartuj kontejner."* Codex si konkrétní cestu v configu ověří, allowlist napíše a kontejner restartuje. Pro víc důvěryhodných lidí přidejte víc ID do seznamu.

  Ověřte si, že to funguje -- požádejte někoho jiného (nebo si vytvořte druhý Telegram účet), ať botovi napíše. Cizí zpráva nesmí dostat odpověď a v logách musí být vidět, že ji bot odmítl.
